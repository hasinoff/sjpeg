...
const uint8_t* Encoder::GetReplicatedSamples(const uint8_t* rgb, int rgb_step,
                                             ...
                                             uint8_t* rep_buf) {
  uint8_t* const dst = (rep_buf == nullptr) ? replicated_buffer_ : rep_buf;
  Replicate8b(rgb, rgb_step, dst, pix_step_ * w, sub_w, sub_h, w, h, pix_step_);
  return dst;
}
...
const uint8_t* Encoder::GetReplicatedYSamples(const uint8_t* in, int step,
                                              ...
                                              uint8_t* rep_buf) {
  uint8_t* const dst = (rep_buf == nullptr) ? replicated_buffer_ : rep_buf;
  Replicate8b(in, step, dst, 16, sub_w, sub_h, 16, 16, 1);
  return dst;
}
...
class Encoder420 final : public Encoder {
 public:
  ...
  void GetSamples(int mb_x, int mb_y, bool clipped, int16_t* out,
                  uint8_t* rep_buf = nullptr) override {
...
class EncoderNV12 final : public Encoder {
 ...
 protected:
  ...
  void GetUVSamples(int mb_x, int mb_y, bool clipped,
    ...
    uint8_t local_tmp_uv[2 * 8 * 8];
    uint8_t* const tmp_uv = (rep_buf == nullptr) ? local_tmp_uv : rep_buf;
    if (clipped) {
